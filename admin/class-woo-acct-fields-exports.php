...
require_once 'parsecsv/parsecsv.lib.php';
  ...
  //   write_parser_log("Finished formating: $formatter->filename");
  ...
  add_action( "woe_csv_print_footer", function($handle, $formatter) {
    ...
    write_parser_log('WOE-OSC ' .$OSCWOE_VERSION .' LOG START');
    write_parser_log("Parsing file: $formatter->filename");
    $csv = new parseCSV($formatter->filename);
    write_parser_log("Parsing fh into csv");
    //write_parser_log($csv);
    $upload   = wp_upload_dir();
    // write_parser_log("Upload dir: ");
    // write_parser_log($upload);
    $upload_dir = $upload['basedir'];
    write_parser_log("Upload basedir: $upload_dir");
    ...
    write_parser_log("Generating new CSV file: ".$newhandle);

    $data = $csv->data;
    write_parser_log("Reading csv data");
    //write_parser_log($data);
    ...
    write_parser_log("Reading newly created parse csv data");
    //write_parser_log($newcsv);
    foreach ($data as $key => $row) {

      //Strip spaces from key names
      $keys = str_replace( ' ', '', array_keys($row) );
      $results = array_combine( $keys, array_values( $row ) );
      // TODO: If $results is not set, then skip to next
      if(isset($results)){

        $order = (object) $results;
        write_parser_log("Exporting order object");
        write_parser_log( $order);
        ...
        // write_parser_log("Reading csv line1");
        // write_parser_log($newcsv->data);
        //Second line - Product
        ...
        // write_parser_log("Reading csv line2");
        // write_parser_log($newcsv->data);
        ...
        if($order->OrderTaxAmount){
          ...
          // write_parser_log("Reading csv line3");
          // write_parser_log($newcsv->data);
          // //fourth line - HST
          $lineHST = new CSVLine();
          ...
          write_parser_log("Reading csv line4");
          //write_parser_log($newcsv->data);
          //TODO: delete tge aparsedfile.csv if it exists.
          // If the user file already exists, delete it
          // sleep(3);
          // if (file_exists($newhandle)) unlink($newhandle);
      }
    ...
    write_parser_log("Final Parsed CSV");
    //write_parser_log($newcsv);


    write_parser_log("writing Parsed CSV into WOE handle: ". $formatter->filename);
    $newcsv->save($formatter->filename);
    //code to check for new content of $formatter
    $postcsv = new parseCSV($formatter->filename);
    write_parser_log("Parsing updated formatter into postcsv");
    write_parser_log("Reading Parsed CSV");
    //write_parser_log($newcsv->data);
    write_parser_log('WOE-OSC ' .$OSCWOE_VERSION .' LOG END');
  }, 10, 2);
